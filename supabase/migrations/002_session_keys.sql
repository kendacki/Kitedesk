...
-- Recreate index with immutable predicate only.
-- Do NOT use expires_at > now() in a partial index predicate.
drop index if exists public.session_keys_user_active_idx;
create index if not exists session_keys_user_active_idx
on public.session_keys (user_smart_wallet, revoked, created_at desc)
where revoked = false;
