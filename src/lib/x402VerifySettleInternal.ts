...
async function settleViaDirectTransfer(
  ...
  try {
    ...
    try {
      const sessionWallet = await getDecryptedSessionKeyWalletBySessionAddress(
        fromNorm,
        provider
      )
    ...
    return {
      ok: false,
      error: `Direct transfer settlement unavailable: ${msg}. Ensure agent wallet has sufficient token balance and correct contract is deployed.`,
    }
  }
}
